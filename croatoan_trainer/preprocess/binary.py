...
class BinaryPreproc(_ClassificationPreproc):
    ...
    Methods:
        ...
        `oversampling(min_count)`:
            Oversamples each class label to reach `min_count` by adding extra
            ids to `self.split` for train.
    ...
    def prepare_targets(self, reverse: bool):
        ...
        Args:
            `reverse` (bool):
                Flag to reverse targets. Hint: it is useful to have more
                samples of `0` class, because usually we are trying to
                optimize F1 metric.
        """
